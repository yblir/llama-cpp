...
// partial sum for each thread
    ...
    for (int i = 0; i < ncols; i += iter_stride) {
        //列坐标，因为一个线程一次for循环迭代处理两个数据，所以32个线程一次迭代可以处理64个数据
        const int col = i + vals_per_iter*tid;
        // x block index ，就是weight数据的第几个block结构体
        const int64_t ib = ((int64_t)row*ncols + col)/qk; // x block index
        // x quant index，当前列坐标所对应的weight数据block结构体内部的第几个索引
        const int iqs = (col%qk)/qr; // x quant index
        // y block start index，与当前weight数据block 对应的input_tensor的起始index
